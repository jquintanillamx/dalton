export const fetchSetCompletionCodeId = async (code, PIM) => {
  try {
    ...
    if (!res.ok) {
      ...
      return false; // Falla por código de estado no exitoso
    }
    ...
    // Si existe un código de error, devolvemos false
    if (result.errorCode) {
    ...
    if (err.message === 'Failed to fetch') {
      console.error('Error de red: no se pudo establecer conexión con el servidor. Verifica la URL o tu conexión.');
    } else {
      console.error(`Error en la petición: ${err.message}`);
    }
    return false; // Siempre regresar false en caso de error
  }
};
